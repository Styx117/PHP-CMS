...
if (isset($_POST['submit'])) 
{
	...
	//The id of the selected page is stored in the cookies and used in the query below to fetch data about that page from the database
	$sct = $_COOKIE["selected"];
	$stmt = "UPDATE pages SET Content = '$body', Menu_name = '$title', image_name = '$file_name' WHERE ID = '$sct';";
	if($conn->query($stmt) === TRUE) {

		echo "Page successfully inserted <br>";
		header("location:index.php");

	} else {
		//if the query was unsuccessful display this and the error itself
		echo "Something went wrong. Page wasn't Updated <br>.\r\n".$conn->error;
...
<body>
	<nav class="navbar navbar-expand-lg navbar-dark bg-dark static-top">
		...
		<div class="container">
			...
			<?php 
			require('db.php');
	//Loop through the pages table and generate a check box for each of them
			$stmt = "SELECT * FROM pages";
			if($result = $conn->query($stmt))
			{
				while($row = $result->fetch_assoc()){
					echo "<br><input type='checkbox' class='radio' value=".$row['ID']." onclick='GetID(this);'>". $row['Menu_name']."<br><br><br><br>";
				}
			}
			else 
			{
		//if the query was unsuccessful display this 
				echo "Could not fetch pages.";
			}
			...
			<form method="POST" enctype="multipart/form-data">
				<div class="form-group">
					<label>Title</label>
					<input type="text" name="title" class="form-control" placeholder="Input a Title" id="myTitle" required>
				</div>
				<div class="form-group">
					<label>Body</label>
					<textarea name="body" class="form-control" cols="30" rows="10"  id="myBody" required>
						<?php 
			//Select the information from the page that is currently selected and fetch its title and content 
						$sct = $_COOKIE["selected"];
						if ($sct == null) 
						{
							$i = 1;
							$stmt = "SELECT * FROM pages WHERE ID = '$i'";
						...
						{
							$stmt = "SELECT * FROM pages WHERE ID = '$sct'";
						}
						?>
					</textarea>
				</div>
				...
				<?php 
		//Put the title and content of the page in the textbox and textarea
				if($result = $conn->query($stmt))
				{
					while($row = $result->fetch_assoc())
					{
						echo "<script>$('#myTitle').val('".$row['Menu_name']."')</script>";
						echo "<script>$('#myBody').val('".$row['Content']."')</script>";
					}
				}
				?>
				Image:
				...
				<br>
				<button name="submit" type="submit"  value="Change" class="btn btn-primary">Change !</button>
